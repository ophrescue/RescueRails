...
class User < ApplicationRecord
  audited except: %i[lastverified password encrypted_password password_reset_token salt password_reset_sent_at], on: [:update]
  ...
  has_many :coordinated_cats, -> { where(status: ['adoptable', 'adopted', 'adoption pending', 'on hold', 'coming soon', 'return pending']) }, class_name: 'Cat', foreign_key: 'coordinator_id'
  ...
  FILTER_FLAGS = { active_volunteer: :active,
                   ...
                   cat_handling_training_complete: :cat_handling_training_complete,
                   dog_adder: :add_dogs,
                   dog_editor: :edit_dogs,
                   dog_handling_training_complete: :dog_handling_training_complete,
