...
feature "folder management", js: true do
  ...
  describe "show a folder" do
    ...
    context 'when the folder contains files' do
      let!(:resource){ create(:attachment, attachment_file_name: "foo.pdf", updated_by_user_id: admin.id, attachable_id: folder.id, attachable_type: 'Folder') }
      ...
      it "should show list of attachments" do
        expect(page).to have_link('.btn-primary', href: "/foo.pdf")
