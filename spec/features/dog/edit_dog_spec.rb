...
feature 'edit a dog' do
  ...
  feature 'permitted access to fields' do
    let(:active_record_attributes){ [:created_at, :updated_at, :id, :wait_list].map(&:to_s) }
    ...
    context 'user has all permissions' do
      ...
      it 'should allow access to all fields' do
        ...
        expect(page.find("#dog_wait_list")).not_to be_disabled
      end
    end
  end
  ...
  feature "change and save attributes" do
    ...
    context 'user enters valid attributes' do
      it "should save the updated values", js: true do
        ...
        find("#new_dog_photo").set(Rails.root.join('spec','fixtures','photo','animal_pic.jpg').to_s)
        click_link('Add another Photo')
        all("#new_dog_photo")[1].set(Rails.root.join('spec','fixtures','photo','animal_pic.jpg').to_s)
        click_link('Add another Photo')
    ...
    context 'user enters invalid attributes --client-side validation', js: true do
      ...
      context 'photo exceeds limit' do
        ...
        it 'should not save and should warn user' do
          click_link('Add a Photo')
          find("#new_dog_photo").set(Rails.root.join('spec','fixtures','photo','animal_large_pic.jpg').to_s)
