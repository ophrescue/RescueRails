    ...
    context 'when a manager is signed in, ' do
      ...
      context 'filter by age' do
        ...
        it 'finds the correct dog' do
          ...
          expect(results[0]).not_to include(senior_dog)
        end
      end
      ...
      context 'filter by multiple ages' do
        ...
        it 'finds all matching dogs' do
          expect(results[0]).to include(baby_dog)
          expect(results[0]).to include(senior_dog)
        end
      end
      ...
      context 'filter by any status' do
        ...
        it 'finds the correct dog' do
          ...
          expect(results[0]).not_to include(other_dog)
        end
      end
      ...
      context 'filter by multiple statuses' do
        ...
        it 'finds multiple correct dogs' do
          expect(results[0]).to include(found_dog)
          expect(results[0]).to include(another_found_dog)
          expect(results[1]).to eq 2
          expect(results[0]).to_not include(other_dog)
        end
      end
      ...
      context 'by flags array with no_cats flag set' do
        ...
        let(:params) { { filter_params:  { has_flags: ["spay_neuter_needed"] } } }
      ...
      context 'by flags array with no_dogs flag set' do
        ...
        let(:params) { { filter_params:  { has_flags: ["no_dogs"] } } }
      ...
      context 'by breed' do
        ...
        it 'finds by primary breed partial match' do
          params = { filter_params: { search_field_index: 'breed', search: 'lab' } }
          results = DogFilter.new(params).filter
          expect(results[0]).to include(primary_breed_dog)
          expect(results[0]).not_to include(secondary_breed_dog)
        end

        it 'finds by secondary breed partial match' do
          params = { filter_params: { search_field_index: 'breed', search: 'terr' } }
          results = DogFilter.new(params).filter
          expect(results[0]).to include(secondary_breed_dog)
          expect(results[0]).not_to include(primary_breed_dog)
        end
      end
